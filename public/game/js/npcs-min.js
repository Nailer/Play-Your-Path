var npc = {
  fish: {
    select: function () {
      var e = $("#fish, #fish_click_area");
      return e;
    },
    click_area: function () {
      $("<div/>", { id: "fish_click_area" }).appendTo("#floor");
    },
    swim: function () {
      if ($.inArray("disappear", fish) === -1) {
        $(npc.fish.select()).sprite({ fps: 8, no_of_frames: 5 });
        npc.fish.click_area();
        npc.fish.move();
        npc.fish.react();
        $("#key").hide();
    ...
    move: function (e) {
      var t = $("#fish");
      e ? t.spToggle().fps(8).spState(2) : t.spState(2);
      var e = function (t) {
        t.animate({ left: 242, top: -236 }, 5e3, "linear", function () {
          t.spState(3);
        }).animate({ left: -129, top: -84 }, 5e3, "linear", function () {
          if (!window.test) {
            t.spState(2);
            e(t);
          }
    ...
    move_to_player: function (e, t) {
      t && e.spToggle();
      e.stop(!0)
        .animate({ left: 98, top: -190 }, 200)
      ...
      $(npc.fish.select()).click(function () {
        room.the_player.go_to.start({
          ...
          action: function () {
            ...
            npc.fish.move_to_player($("#fish"), !0);
  ...
  furnace: {
    ...
    burn: function () {
      function t() {
        e.animate({ opacity: 0.8 }, 200)
          .animate({ opacity: 0.6 }, 250)
          .animate({ opacity: 0.85 }, 200)
          .animate({ opacity: 0.65 }, 100)
          .animate({ opacity: 0.9 }, 150)
          .animate({ opacity: 0.65 }, 200, function () {
            t();
          });
      }
      var e = npc.furnace.fire();
      t();
    },
    has_food: function () {
      if (
        $.inArray("twig", collected) !== -1 &&
        $.inArray("twig", used) === -1
      ) {
        dialogue_box.display(
          {
        ...
        $.inArray("note", collected) !== -1 &&
        $.inArray("note", used) === -1 &&
        $.inArray("scene_keypad", played) === -1
      ) {
        dialogue_box.display(
          {
            character: !1,
        ...
        n = function () {
          setTimeout(function () {
            if (
              ...
              npc.furnace.furnace().text_cloud("I want more!", 1e3);
              scene.no_click(!1);
            } else npc.furnace.is_strong();
          }, 500);
        };
      t.animate({ opacity: 0.5 }, 375).animate({ opacity: 0 }, 375);
      Modernizr.csstransitions
        ? e
            .transition({ top: "-=80" }, 250)
            .transition({ top: "+=80" }, 100, function () {
    ...
    is_strong: function () {
      var e = npc.furnace.furnace(),
        t = npc.furnace.body(),
        n = npc.furnace.eyes(),
        r = npc.furnace.mouth(),
        i = npc.furnace.fire();
      e.text_cloud("YEAH!", 2e3);
      r.attr("style", "");
      if (!Modernizr.csstransitions) {
        t.css({ rotate: "rotate(0deg)" });
        t.animate({ top: 15 }, 500, function () {
          sound_screech.play();
        })
          .animate({ top: -3 }, 100)
          .delay(300)
        ...
        t.transition({ top: 15, rotate: 0 }, 500, function () {
          sound_screech.play();
        })
      ...
      r.animate({ top: 435 }, 700)
        ...
        .animate({ top: 515 }, 100, function () {
          ...
          e.text_cloud("Thank you man!", 2e3);
          setTimeout(function () {
            e.text_cloud("Check your room! Things have changed!", 2e3);
          }, 3e3);
          npc.furnace.burn();
          npc.furnace.random_moves();
          var t = $.jStorage.get("played");
          t.push("scene_furnace");
          $.jStorage.set("played", t);
    ...
    is_weak: function () {
      ...
      scene.no_click(!0);
      $.inArray("twig", used) === -1
        ? e.text_cloud("I'm weak...", 2e3)
        : $.inArray("note", used) === -1
        ? e.text_cloud("Help me please...", 2e3)
        : $.inArray("coal", used) === -1 &&
          e.text_cloud("Still hungry...", 2e3);
      i.animate({ opacity: 0.5 }, 1700).animate(
        { opacity: 0 },
        300,
        function () {
          scene.no_click(!1);
      ...
      sound_screech.play();
      if (!Modernizr.csstransitions) {
        t.animate({ top: 5, rotate: 1 }, 1700)
          .animate({ top: 10, rotate: -0.5 }, 100)
          .animate({ top: 7 }, 50)
          .animate({ top: 10 }, 50)
          .animate({ top: 8 }, 50)
          .animate({ top: 10 }, 50);
        r.animate({ top: "-=10" }, 1700)
        ...
        t.transition({ top: 5, rotate: 1 }, 1700)
          .transition({ top: 10, rotate: -0.5 }, 100)
          .transition({ top: 7 }, 50)
          .transition({ top: 10 }, 50)
          .transition({ top: 8 }, 50)
          .transition({ top: 10 }, 50);
        r.transition({ top: "-=10" }, 1700)
    ...
    random_moves: function () {
      var e = npc.furnace.body();
      window.timer = setInterval(function () {
        var t = Math.random() * 1;
        t > 0.5
          ? e
              .transition({ rotate: 0.5 }, 500)
